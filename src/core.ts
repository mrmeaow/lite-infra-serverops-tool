...
if (!VOL_DIR || VOL_DIR.length < 0 || VOL_DIR.trim() === "")
  throw new Error("Invalid or missing VOL_DIR value");
...
export async function isContainerRunning(containerName: string): Promise<boolean> {
  try {
    ...
    return (
      code === 0 &&
      (stdout.trim() === containerName || stdout.trim().split("/")[0].includes(containerName))
    );
  } catch {
    return false;
  }
...
export async function startService(
  ...
  try {
    ...
    if (service.type === "standalone") {
      ...
      const result = await execCommand("docker compose", ["-f", composeFile, "up", "-d"], {
        stdio: "inherit",
      });
...
export async function stopService(
  ...
  try {
    if (service.type === "standalone") {
      ...
      const result = await execCommand("docker compose", ["-f", composeFile, "down"], {
        stdio: "inherit",
      });
...
export async function getServiceLogs(serviceName: string, lines: number = 50): Promise<string> {
  const service = SERVICES[serviceName];
  if (!service) {
